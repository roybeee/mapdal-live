...
const r=id?DR.find(x=>x.id===id):null;const v=r||{on:true,chart_note:true,buy_label:'구매하기',schedule:[],options:[],winners:[]};
 ...
 <b>발표 공지문<br>(본문·이미지)</b><span><span class="hint">당첨자 발표 페이지에 <b>제목 아래 공지문</b>으로 표시되며, <b>발표 전(발표 예정 화면)에도 표시</b>됩니다. [발표완료] 전환은 발표 시각 도달 + 당첨 명단 저장 시 이뤄집니다. 줄바꿈은 그대로 반영되고, 이미지를 올리면 커서 위치와 무관하게 본문 끝에 삽입됩니다. 비우면 섹션이 숨겨집니다.</span>
