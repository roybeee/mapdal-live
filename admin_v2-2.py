...
"""
import os, re, json, sqlite3, base64, hashlib, hmac, secrets, datetime
...
def get_actor(request: Request):
    ...
    mt = admin_token()
    if not tok: raise HTTPException(403, 'forbidden')
    if mt and tok == mt:
        try: ensure_ready()
        except Exception: pass
    ...
    row = one('SELECT * FROM admins WHERE token_hash=? AND active=1', (h,))
    if not row: raise HTTPException(403, 'forbidden')
    return {'name': row['name'], 'role': row['role'], 'master': False, 'token': tok, 'id': row['id']}
...
def api_admin_create(request: Request, body: dict = Body(...)):
    ...
    token = 'mpd_' + secrets.token_urlsafe(15)
    run('INSERT INTO admins VALUES(?,?,?,?,1,?)', (uid(), name, role, hashlib.sha256(token.encode()).hexdigest(), now_iso()))
    audit(a, '관리자발급', name, role)
...
@admin_router.post('/admin/api/admins/reissue')
def api_admin_reissue(request: Request, body: dict = Body(...)):
    a = get_actor(request); need(a, 3, '관리자 관리')
    r = one('SELECT * FROM admins WHERE id=?', (body.get('id'),))
    if not r: raise HTTPException(404, 'not found')
    token = 'mpd_' + secrets.token_urlsafe(15)
    run('UPDATE admins SET token_hash=? WHERE id=?', (hashlib.sha256(token.encode()).hexdigest(), r['id']))
    audit(a, '토큰재발급', r['name'], '')
    return {'ok': True, 'token': token}
...
def dashboard(request: Request):
    try:
        ...
        return HTMLResponse('<meta charset=utf-8><body style="font-family:sans-serif;padding:40px">'
                            '<h2>MAPDAL 관리자</h2><p>접근 권한이 없습니다. 주소 뒤에 '
                            '<code>?token=관리자토큰</code> 을 붙여 접속하세요.</p>', status_code=403)
    html = ADMIN_HTML.replace('__ACTOR__', json.dumps({'name': actor['name'], 'role': actor['role']}, ensure_ascii=False))
    resp = HTMLResponse(html)
    resp.set_cookie('mp_admin', actor['token'], httponly=True, samesite='lax', secure=True, max_age=604800)
    return resp
...
<header><h1>MAPDAL<span>SEOUL</span></h1><span class="who" id="who"></span><nav id="nav"></nav></header>
<main>
...
<section id="t-admins" style="display:none">
  ...
  <div class="toolbar" style="margin-top:12px"><input id="aname" placeholder="이름 (예: 김스태프)">
  ...
  <div class="hint">마스터 토큰(Render 환경변수 ADMIN_TOKEN)은 항상 유효하며 여기서 관리되지 않습니다. 발급된 토큰은 생성 순간 한 번만 표시되니 안전하게 전달하세요.</div></div>
...
async function loadAdmins(){try{const d=await api('/admin/api/admins');
 $('#alist').innerHTML=`<table><tr><th>이름</th><th>역할</th><th>상태</th><th>발급일</th><th></th></tr>
 ${d.rows.map(r=>`<tr><td><b>${esc(r.name)}</b></td><td>${RN[r.role]||esc(r.role)}</td>
 ...
 <button class="btn sm ghost" onclick="reissue('${r.id}')">토큰 재발급</button></td></tr>`).join('')||'<tr><td colspan=5 class="loading">발급된 계정 없음</td></tr>'}</table>`;
...
async function createAdmin(){const name=$('#aname').value.trim();if(!name)return toast('이름을 입력하세요');
 try{const r=await api('/admin/api/admins/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,role:$('#arole').value})});
 $('#mbox').innerHTML=`<h3>계정 발급 완료 — ${esc(name)}</h3><p>아래 접속 주소를 <b>지금 복사해 전달</b>하세요. 이 화면을 닫으면 다시 볼 수 없습니다 (재발급만 가능).</p>
 <div class="tokenbox">https://mapdal.kr/admin/dashboard?token=${esc(r.token)}</div>
 <div style="text-align:right"><button class="btn" onclick="closeM();loadAdmins()">확인</button></div>`;
 $('#mbg').style.display='flex';$('#aname').value=''}catch(e){toast(e.message)}}
...
async function reissue(id){if(!confirm('기존 토큰은 즉시 무효화됩니다. 재발급할까요?'))return;
 try{const r=await api('/admin/api/admins/reissue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
 $('#mbox').innerHTML=`<h3>토큰 재발급 완료</h3><div class="tokenbox">https://mapdal.kr/admin/dashboard?token=${esc(r.token)}</div>
 <div style="text-align:right"><button class="btn" onclick="closeM()">확인</button></div>`;$('#mbg').style.display='flex'}catch(e){toast(e.message)}}
